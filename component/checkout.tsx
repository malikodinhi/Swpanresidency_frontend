...
const billingCalculations = useMemo(() => {
  ...
  const checkIn = new Date(
    `${selectedRecord.checkin_date}T${selectedRecord.checkin_time}`
  );
  ...
  const diffHours =
    Math.max(0, checkOut.getTime() - checkIn.getTime()) /
    (1000 * 60 * 60);

  const totalDays = Math.max(1, Math.ceil(diffHours / 24));

  const subtotal = totalDays * currentRoomRent;

  const tax = subtotal * 0.36;
  ...
  const advance = Number(selectedRecord.advance_amount || 0);
  ...
  const handleExportToExcel = () => {
    ...
    const headers = ["Room No", "Guest Name", "Mobile No", "Checkout Date", "Checkout Time", "Duration (Days)", "Settled Amount (INR)","Total Amount","GST", "Payment Mode", "Remarks"];
    ...
    const rows = filteredCheckouts.map((out) => {
      ...
      return [
        ...
        Number(out.grand_total).toFixed(2),
         Number(out.gst).toFixed(2),
        `"${out.pay_mode}"`,
        `"${remarksText}"`
      ];
    });
  ...
  <span>SGST & CGST </span>
  <span>
    ₹{receiptData.financial_breakdown.gst.toFixed(2)}
  </span>
</div>
...
<div className="flex justify-between text-green-400 font-bold">
  ...
  <span>
    ₹{receiptData.financial_breakdown.grand_total.toFixed(2)}
  </span>
</div>
  ...
  <div className="w-full bg-slate-900 p-3 rounded-xl border border-yellow-500/20 font-bold text-yellow-400">
...
<div>
  ...
  <div className="w-full bg-slate-900 p-3 rounded-xl border border-green-500/20 font-bold text-green-400">
    ₹{billingCalculations.grandTotal.toFixed(2)}
  </div>
</div>
                <div>
                ...
                <div>
                  ...
                  >
                    ₹{billingCalculations.balanceAmount.toFixed(2)}
                  </div>
                </div>
              </div>
